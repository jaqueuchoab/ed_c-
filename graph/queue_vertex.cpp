...
bool QueueVertex::isEmpty() const
{
  return (front == NULL);
}

// Enfileirar elementos
void QueueVertex::enqueueVertex(VertexType newItem)
{
  ...
  if (!isFull())
  {
    ...
    NodeTypeVertex *newNode = new NodeTypeVertex;
...
VertexType QueueVertex::dequeueVertex()
{
  ...
  if (!isEmpty())
  {
    ...
    tempPont = front;
    // Criando var item do tipo VertexType (tipo que a fila guarda) e atribuindo a ela o valor guardado no front info (primeiro elemento da fila)
    VertexType item = front->item;
  ...
  else
  {
    ...
    throw "QueueVertex is empty!";
  }
}

void QueueVertex::print() const
{
  // Ponteiro temporário
  NodeTypeVertex *tempPont = front;
  if (!isEmpty())
  {
